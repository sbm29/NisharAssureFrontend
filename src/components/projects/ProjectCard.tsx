...
const ProjectCard: React.FC<ProjectCardProps> = ({ project }) => {
  ...
  return (
    <Link to={`/projects/${id}`} className="block h-full">
      <Card className="h-full hover:shadow-md transition-shadow duration-300 hover:scale-[1.01] cursor-pointer">
        <CardHeader className="pb-2  ">
          <div className="flex justify-between items-start">
            <CardTitle className="text-lg font-medium">
        ...
        <CardContent className="pb-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              <span>Latest Run Pass Rate</span>
              <span
                className={`font-medium ${
                  latestRunPassRate >= 70 ? "text-green-600" : "text-amber-600"
                }`}
              >
                {latestRunPassRate !== null ? `${latestRunPassRate}%` : "N/A"}
              </span>
            </div>
          </div>
        </CardContent>
